...
beforeAll(async () => {
  await resetTestDB(); // Очищаем БД перед началом тестов
...
// Здесь начнём писать тесты по блокам (создание, получение, подача, отмена, удаление и т.д.)
...
describe('GET /api/events - Retrieving active events', () => {
  test('✅ Should return an array of active events', async () => {
    ...
    // Проверяем, что у каждого события есть нужные поля
...
describe('POST /api/events/:eventId/apply - Applying to events', () => {
  const eventId = 1; // используем уже созданное событие
  ...
  test('❌ User from another shift cannot apply', async () => {
    const res = await request(app)
        .post(`/api/events/${eventId}/apply`)
        .set('Authorization', `Bearer ${user2Token}`); // уже готовый пользователь из 2nd shift
...
describe('DELETE /api/events/:eventId/cancel - Cancelling application', () => {
  const eventId = 1; // Team Meeting, на который user1 уже подал заявку
  ...
  test('❌ User cannot cancel application if none exists', async () => {
    const res = await request(app)
      .delete(`/api/events/1/cancel`) // допустим, заявка на eventId 1 уже отменена
      .set('Authorization', `Bearer ${user1Token}`);
...
describe('DELETE /api/events/:eventId - Deleting events', () => {
  const eventId = 1;
  const existingEventId = eventId; // используем в негативных тестах
  ...
  test('❌ User cannot delete any event', async () => {
    ...
    const res = await request(app)
        .delete(`/api/events/${eventId}`) // eventId создан менеджером1
